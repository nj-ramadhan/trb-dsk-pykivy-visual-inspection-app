...
class ScreenMain(MDScreen):   
    ...
    def regular_update_display(self, dt):
        ...
        try:
            ...
            if dt_user != '':
                ...
                screen_antrian_new.ids.img_user.source = f'https://{FTP_HOST}/simpkb909012/foto_user/{dt_foto_user}'
            else:
    ...
    def exec_reload_table(self):
        ...
        try:
            ...
            tb_antrian.execute(f"SELECT id, noantrian, nopol, nouji, statusuji, merk, type, idjeniskendaraan, jbb, bahan_bakar, warna, check_flag FROM {TB_DATA}")
...
class ScreenMenu(MDScreen):
    ...
    def exec_verify_data(self):
        ...
        try:
            ...
            toast_msg = f'Gagal Membuat Tabel Daftar Berkala karena Data Sudah Ada'
            toast(toast_msg)
            print(toast_msg, e)
    ...
    def exec_verify_payment(self):
        ...
        try:
            ...
            make_dir_path = f'/var/www/simpkb909012/capture/{today}/{dt_sts_uji}-{dt_no_antri}'
...
class ScreenAntrianNew(MDScreen):
    ...
    def exec_find(self):
        ...
        try:
            ...
            toast_msg = f'Berhasil Menemukan Data Master dari nomor polisi {dt_find_no_pol} atau nomor uji {dt_find_no_uji}'
            toast(toast_msg)
            print(toast_msg)
    ...
    def exec_fetch_master_data(self, dt_find_no_pol, dt_find_no_uji):
        ...
        try:
            ...
            if myresult is None:
                ...
                if month_to_add > 12:
                    month_to_add -= 6
                    new_last_uji_date = last_uji_date.replace(year=last_uji_date.year + 1)
                else:
                    new_last_uji_date = last_uji_date.replace(month=month_to_add)
                dt_temp_tgl_uji_habis = str(new_last_uji_date.strftime('%d-%m-%Y'))
                    
                print(f"Last Uji: {dt_temp_tgl_uji_terakhir}, Uji Habis: {dt_temp_tgl_uji_habis}")
...
class ScreenInspectId(MDScreen):        
    ...
    def exec_save(self):
        ...
        dt_tgl_baru_uji = str(time.strftime("%Y/%m/%d", time.localtime()))
        dt_tgl_waktu_baru_uji = str(time.strftime("%Y/%m/%d %H:%M:%S", time.localtime()))
        ...
        try:
            ...
            tb_uji = mydb.cursor()
            tb_uji.execute(f"SELECT tanggal FROM {TB_UJI} WHERE nopol = '{dt_no_pol}' ORDER BY tanggal DESC LIMIT 1")
            result_tb_uji = tb_uji.fetchone()
            mydb.commit()  
            dt_tgl_terakhir_uji = result_tb_uji[0].strftime("%Y/%m/%d")
            print(dt_tgl_terakhir_uji, dt_tgl_baru_uji)

            if dt_tgl_terakhir_uji == dt_tgl_baru_uji:
                toast_msg = f'Gagal Menambahkan Baris Data Kode Kelompok Uji V1 ke Database Tabel Uji, Data Sudah Ada'
                toast(toast_msg)
                print(toast_msg)
            
            else:
                mycursor = mydb.cursor()
                sql = f"INSERT INTO {TB_UJI} (tanggal, nopol, noantrian, nouji, newnouji, kode_kelompok_uji, lulus_uji, id_image) VALUES ('{dt_tgl_waktu_baru_uji}', '{dt_no_pol}', '{dt_no_antri}','{dt_no_uji}','{dt_no_uji}','V1','1', '{id_image}' )"
                mycursor.execute(sql)
                mydb.commit()  
                toast_msg = f'Berhasil Menambahkan Baris Data Kode Kelompok Uji V1 ke Database Tabel Uji'
                toast(toast_msg)
                print(toast_msg)

        except Exception as e:
            toast_msg = f'Gagal Menambahkan Baris Data Kode Kelompok Uji V1 ke Database Tabel Uji'
            toast(toast_msg)
            print(toast_msg, e)
        ...
        try:
            ...
            if self.ids[f'bt_komponen_uji{selected_row_komponen_uji}'].icon == "cancel":
                ...
                mycursor = mydb.cursor()
                sql = f"UPDATE {TB_UJI} SET lulus_uji = '1' WHERE nopol = '{dt_no_pol}' AND id_image = '{id_image}' "
                mycursor.execute(sql)
                mydb.commit()
                
            toast_msg = f'Berhasil Memperbaharui Data di Database Tabel Uji'
            toast(toast_msg)
            print(toast_msg)
        ...
        try:
            tb_uji = mydb.cursor()
            tb_uji.execute(f"SELECT id_uji FROM {TB_UJI} WHERE nopol = '{dt_no_pol}' AND kode_kelompok_uji = 'V1' ORDER BY tanggal DESC LIMIT 1")
            result_tb_uji = tb_uji.fetchone()
            mydb.commit()
            ...
            for i in range(db_subkomponen_uji[0,:].size):                
                ...
                tb_subkomponenuji = mydb.cursor()
                tb_subkomponenuji.execute(f"SELECT id_uji_detail FROM {TB_UJI_DETAIL} WHERE id_uji = '{id_uji}' AND kode_subkomponen_uji = '{kode_subkomponen_uji}' LIMIT 1")
                result_tb_subkomponenuji = tb_subkomponenuji.fetchone()
                mydb.commit()  

                if result_tb_subkomponenuji is None:
                    mycursor = mydb.cursor()
                    sql = f"INSERT INTO {TB_UJI_DETAIL} (id_uji, hasil, kode_komponen_uji, kode_subkomponen_uji, komentar) VALUES ('{id_uji}', '{str(int(flags_subkomponen_uji[i]))}','{dt_selected_kode_komponen_uji}','{kode_subkomponen_uji}','{comment_subkomponen_uji}')"
                    mycursor.execute(sql)
                    mydb.commit()
                else:
                    mycursor = mydb.cursor()
                    sql = f"UPDATE {TB_UJI_DETAIL} SET hasil = '{str(int(flags_subkomponen_uji[i]))}', komentar = '{comment_subkomponen_uji}' WHERE id_uji = '{id_uji}' AND kode_subkomponen_uji = '{kode_subkomponen_uji}' "
                    mycursor.execute(sql)
                    mydb.commit()

            toast_msg = f'Berhasil Menyimpan Data ke Database Tabel Uji Detail'
            toast(toast_msg)        
            print(toast_msg)    
...
class ScreenInspectPit(MDScreen):        
    ...
    def exec_save(self):
        ...
        try:
            ...
            if dt_tgl_terakhir_uji == dt_tgl_baru_uji:
                toast_msg = f'Gagal Menambahkan Baris Data Kode Kelompok Uji V2 ke Database Tabel Uji, Data Sudah Ada'
                toast(toast_msg)
                print(toast_msg)
            
            else:
                mycursor = mydb.cursor()
                sql = f"INSERT INTO {TB_UJI} (tanggal, nopol, nouji, newnouji, kode_kelompok_uji, lulus_uji, id_image) VALUES ('{dt_tgl_baru_uji}', '{dt_no_pol}','{dt_no_uji}','{dt_no_uji}','V2','1', '{id_image}' )"
                mycursor.execute(sql)
                mydb.commit()  

        except Exception as e:
            toast_msg = f'Gagal Menambahkan Baris Data Kode Kelompok Uji V2 ke Database Tabel Uji'
            toast(toast_msg)
            print(toast_msg, e)
...
class ScreenRealtimeCctv(MDScreen):        
    ...
    def resize_and_crop(self, img, size=600):
        h, w = img.shape[:2]
        # Scale so the smaller side fits 'size'
        scale = size / min(h, w)
        ...
        y1 = (nh - size) // 2
        x1 = (nw - size) // 2
        cropped_img = img_resized[y1:y1+size, x1:x1+size]
        return cropped_img

    def save_jpeg_with_target_size(self, img, path, target_min_kb=50, target_max_kb=70):
        # Try different JPEG quality to fit the size range
        quality = 85
        for q in range(85, 30, -5):
            is_success, buf = cv2.imencode('.jpg', img, [int(cv2.IMWRITE_JPEG_QUALITY), q])
            if not is_success:
                continue
            ...
            if target_min_kb <= size_kb <= target_max_kb:
                ...
                return True
            quality = q
        # If not in range, save with the lowest tried quality
    ...
    def exec_save(self):
        ...
        try:
            ...
            upload_dir_path = f'/var/www/simpkb909012/capture/{today}/{dt_sts_uji}-{dt_no_antri}/{dt_no_pol}-{dt_selected_camera + 1}.jpg'

            # Resize to 600x600 before saving
            resized_img = self.resize_and_crop(self.image_cctv, 600)
            # Save with target file size
            self.save_jpeg_with_target_size(resized_img, local_path, 50, 70)
...
class ScreenRealtimePit(MDScreen):        
    ...
    def exec_save(self):
        ...
        try:
            today = str(time.strftime("%Y-%m-%d", time.localtime()))
            local_path = f'assets/images/{dt_no_pol}-{dt_selected_camera + 1}.jpg'
            upload_dir_path = f'/var/www/simpkb909012/capture/{today}/{dt_sts_uji}-{dt_no_antri}/{dt_no_pol}-pit-{dt_selected_camera + 1}.jpg'
